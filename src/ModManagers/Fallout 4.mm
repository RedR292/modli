...
checkTrivialMod() {
	...
	if [ $? -eq 0 ]; then
		echo "modli: WARNING: This mod may be an ENB mod and cannot be installed as a raw mod. It must be installed with a custom script" >&2
		return 1
	fi
	##Check if MOD is a script extender
	7z l "$MOD" | grep -q 'f4se_loader.exe'
	if [ $? -eq 0 ]; then
		echo "modli: WARNING: this is a script extender and cannot be installed as a raw mod. It must be installed with a custom script" >&2
		return 1
	fi
...
validatemod() {
	##Matches 'yyyy-mm-dd hh:mm:ss (D or '.')' at the start of each line. Example match: 2000-01-01 00:00:00 .
	##Used to isolate files from the directories
...
case $cmd in
	-i)
		...
		if [[ $? -ne 0 ]]; then
			...
			echo "modli: $(basename "$MOD") was installed to $GAME_NAME"
		fi
		shift 2
		;;
	-u)
	...
	-r)
		...
		if [[ $? -ne 0 ]]; then
			...
			echo "modli: $mod was removed from $GAME_NAME's stage"
		fi
		return 0
		shift 2
		;;
